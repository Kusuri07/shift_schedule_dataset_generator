...
except Exception:  # HTML parsing is optional; regex fallback still works
    BeautifulSoup = None
...
# Held-out layout families.  They are generated by generate_ood_layouts.py and
# must never be added to the default training template cycle.
OOD_TEMPLATE_IDS = [
    'ood_name_group_swapped',
    'ood_multi_header',
    'ood_summary_left',
    'ood_irregular_columns',
]
...
class ScheduleRecord:
    ...
    training_objects: list[dict[str, Any]] = field(default_factory=list)
...
class GeneratorConfig:
    ...
    schedule_id_prefix: str = 'schedule'
...
def schedule_day_start_col_1based(schedule: ScheduleRecord) -> int:
    if schedule.template_id == 'ood_summary_left':
        return 7
    if schedule.template_id == 'ood_name_group_swapped':
        return 3
...
def make_group_labels(template_id: str, people_count: int) -> list[str]:
    if template_id in {'grouped_hospital', 'parted_pdf', 'ood_name_group_swapped'}:
...
def generate_schedule(
    ...
    for row_index in range(people_count):
        ...
        rows.append(PersonRow(
            row_id=f'{config.schedule_id_prefix.upper()}_{schedule_index:04d}_R{row_index + 1:03d}',
            name=full_name,
            given_name=given,
    ...
    schedule_id = f'{config.schedule_id_prefix}_{schedule_index:04d}'
...
def export_annotations(schedules: list[ScheduleRecord], output_dir: Path) -> None:
    ...
    objects_jsonl: list[dict[str, Any]] = []
    for schedule in schedules:
        ...
        for training_object in schedule.training_objects:
            item = dict(training_object)
            item['image_path'] = schedule.clean_image_path
            item['image_width'] = schedule.image_width
            item['image_height'] = schedule.image_height
            objects_jsonl.append(item)

    write_jsonl(output_dir / 'annotations' / 'rows.jsonl', rows_jsonl)
    write_jsonl(output_dir / 'annotations' / 'cells.jsonl', cells_jsonl)
    write_jsonl(output_dir / 'annotations' / 'objects.jsonl', objects_jsonl)
    ...
    for item in cells_jsonl:
        cell_csv.append({**item,
                         ...
                         'cell_polygon': json.dumps(item['cell_polygon']),
                         'text_polygon': json.dumps(item['text_polygon']),
                         'name_cell_polygon': json.dumps(item['name_cell_polygon'])})
    write_csv(output_dir / 'annotations' / 'cells.csv', cell_csv)

    object_csv = []
    for item in objects_jsonl:
        object_csv.append({**item,
                           'bbox_px': json.dumps(item['bbox_px']),
                           'cell_polygon': json.dumps(item['cell_polygon']),
                           'text_polygon': json.dumps(item['text_polygon'])})
    write_csv(output_dir / 'annotations' / 'objects.csv', object_csv)
...
def render_dataset_workbook(
    ...
    *,
    ...
    for schedule in schedules:
        ...
        schedule.training_objects = rendered.get('training_objects', [])
        mask_relative = rendered.get('glyph_mask_path')
        if mask_relative:
            from PIL import Image
            from shift_ocr.text_polygons import max_corner_distance, polygon_from_pillow_mask

            mask_path = output_dir / mask_relative
            with Image.open(mask_path) as mask_image:
                for item in schedule.training_objects:
                    try:
                        mask_polygon = polygon_from_pillow_mask(mask_image, item['bbox_px'])
                    except ValueError:
                        continue
                    if max_corner_distance(item['text_polygon'], mask_polygon) > 2.0:
                        item['text_polygon'] = mask_polygon
                        item['text_polygon_source'] = 'glyph_mask'
                    item['text_polygon_validation_max_error_px'] = max_corner_distance(
                        item['text_polygon'], mask_polygon
                    )
            by_cell = {
                (item.get('row_id'), item.get('day')): item
                for item in schedule.training_objects
                if item.get('object_type') == 'shift_code'
            }
            for annotation in schedule.cell_annotations:
                source = by_cell.get((annotation.get('row_id'), annotation.get('day')))
                if source:
                    annotation['text_polygon'] = source['text_polygon']
                    annotation['text_polygon_source'] = source['text_polygon_source']
                    annotation['text_polygon_validation_max_error_px'] = source.get(
                        'text_polygon_validation_max_error_px', 0.0
                    )
            mask_path.unlink(missing_ok=True)
            mask_directory = mask_path.parent
            if mask_directory.exists() and not any(mask_directory.iterdir()):
